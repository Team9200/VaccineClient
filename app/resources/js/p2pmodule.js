...
function sendFile(sendPath){

    parallel([
        (cb) => {
            PeerId.createFromJSON(require('..\\peer\\nodeInfo.json'), (err, senderPeerId) => {
            if (err) { throw err }
            cb(null, senderPeerId)
            })
        },
        (cb) => {
            PeerId.createFromJSON(require('..\\peer\\storageInfo.json'), (err, receiverPeerId) => {
            if (err) { throw err }
        ...
        receiverSN.multiaddrs.add('/ip4/192.168.142.141/tcp/10333') //Storage IP
